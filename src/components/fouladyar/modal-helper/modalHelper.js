...
const ModalHelper = ({open, onClose, onOpen, size, component}) => {
    ...
    function ModalForMobile(){
        return(
            <BottomSheet className={classes.bottomSheet} onDismiss={onClose} style={{zIndex: 100000}} open={open}>
                {component}
            </BottomSheet>
        )
    }
    ...
    function ModalForWeb(){
        return(
            <Modal style={themeState.lng === 'fa' ? {direction: "rtl"} : {direction: "rtl"}} isOpen={open} size={size ? size : "md"} toggle={open ? onClose : onOpen}>
                {component}
            </Modal>
        )
    }
