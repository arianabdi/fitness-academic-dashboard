...
export const ChatItemHeader = ({ item }) => {
  ...
  //   "muted": false,
  //   "type": "PRIVATE",
  //   "lastMessage": "به خدا فاکتور هاتو فرستادم",
  //   "lastMessageAt": "2023-10-09T15:03:10.107Z",
  //   "userfirstname": "jon",
  //   "userlastname": "doe",
  //   "userusername": "sahand.z",
  //   "unreadmessages": "1"
  // }
  return (
    <Link to={`${process.env.PUBLIC_URL}/app-chat`} className="chat-link">
      ...
      {item.group === true ? (
          ...
          {item.user.slice(0, 2).map((user, idx) => {
            return (
              <UserAvatar
                key={idx}
                theme={user.theme}
                text={findUpper(`${user.userfirstname} ${user.userlastname}`)}
                image={user.image}
        ...
        <UserAvatar theme={item.theme} text={findUpper(`${item.userfirstname} ${item.userlastname}`)} image={item.image} className="chat-media">
